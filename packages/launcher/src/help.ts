 ...
 *
 * `install` and `add` are launcher commands: they never reach the engine, so the
 * engine's help — which is citty's, generated from the commands it actually has —
 * cannot list them, and used to leave `penv --help` naming twenty-one commands
 * while refusals told the reader to run a twenty-second. `penv add --help` was
 * refused outright, under a root help that promised `penv <command> --help`.
 *
...
import { EXTENSIONS_PATH, MANIFEST_PATH } from "@penvhq/core";
import { INSTALL_COMMAND, LOCAL_FLAG, MIN_PACKAGE_AGE_DAYS, TRUST_YOUNG_FLAG } from "./errors.js";
