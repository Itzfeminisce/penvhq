...
- A **global launcher** (`penv`) and a project-pinned **engine** (`@penvhq/cli`). `.penv/state/manifest.json` is the committed contract naming the exact engine and extension versions with their integrity; the launcher resolves them from `$PENV_HOME` and delegates. A newer launcher never upgrades a project — `penv upgrade [version]` does, moving the manifest and the runtime dependency together. One user-visible version: `penv --version` prints one line.
