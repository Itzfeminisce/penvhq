...
**Decision (2026-07-19).** The `import "@penvhq/penv/config"` mirror of the tree onto `process.env` is promoted from compat-only to a blessed delivery surface, with three properties: it validates through `load()` before writing; it writes each parameter under the same generated variable `generate` and `push` produce (`override` bending names for all three); and it is **exclusive over the schema** — every schema-declared parameter is penv's ambiently, written when it resolves and *deleted* when it does not, while undeclared variables are never touched. `penv init` places the import at the framework's own guaranteed pre-app seam (Next's `instrumentation.ts`, a Nitro plugin, `hooks.server.ts`, `node --import`), detected at init and never stored (config records decisions, not identities).
...
**Why the seam, and not import discipline.** The mirror only helps if it runs before the SDK module reads `process.env`, and module-graph order is not something a developer controls — one added import anywhere can silently reorder evaluation. That fragility is exactly why the import was compat-only. The fix is not care, it is placement: every framework ships a hook it *guarantees* runs before app code, and `init` scaffolds the import there. The ordering hazard is not managed; it is deleted. This is also v0.7's concept finishing its arc: the ambient surface is a *projection* — resolved values delivered downstream so something that reads `process.env` can run — landing in the local process instead of in GitHub Actions.

**Rejected.** *Mirroring by default, without the import* — a library consumer who never asked for ambient writes must never get them. *A per-SDK adapter registry* ("penv knows WorkOS's keys") — penv would be curating other products' env surfaces forever; the schema plus `override` expresses the same fact as the user's own declaration. *A lazy `process.env` proxy as the primary mechanism* — true ordering independence, but edge runtimes and env-snapshotting hosts break `process.env` reassignment, and a delivery surface that works on some platforms is a silent downgrade on the rest; it stays a possibility behind the eager mirror, not the mechanism.

**Accepted cost, stated not hidden.** Every schema-declared value becomes readable by any code in the process — which is the dotenv world's baseline, restated rather than worsened, and the reason mirroring stays behind an explicit import. And exclusivity means a platform-injected variable that a schema declares is overridden or deleted by penv's truth; that is the point, and it is also a behavior change a migrating team must hear about, so `doctor`'s `ambient-shadow` check names every ambient key the mirror will own.
