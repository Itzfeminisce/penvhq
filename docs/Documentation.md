...
**`penv upgrade [version]` is what moves it.** It resolves the release from npm — no version means the latest — and writes the manifest's pin with the integrity npm published for those exact bytes, never one computed on your machine. The project's `@penvhq/penv` moves to the same exact version through the package manager your repository already uses — and in a workspace that means every `package.json` declaring it, because a package holding its own older copy is an older bridge running under the pin. Every change is shown before any of it happens, and one answer covers all of them: decline, and not one file is touched. Extensions keep their own pins — `penv add <package>@<version>` moves one.
...
`add` resolves the exact version, records it in `.penv/state/manifest.json` with its integrity hash, installs it into the launcher's cache, imports it once to prove penv can load it — a package whose entry is TypeScript source is refused here rather than days later, from an unrelated command — writes a committed type-only declaration under `.penv/state/extensions/`, and offers — never assumes — the one-line edit to `penv.config.ts` and whatever onboarding step the provider declares (`penv cloud login`, for instance). Your `package.json` is untouched.

**Where the engine looks for one**, in this order: the local-extension list, then your project's own `node_modules`, then `$PENV_HOME` at the version the manifest pins. A copy the project itself depends on wins over a pinned release, so a repository developing a provider runs the copy it is editing; a pinned extension missing from `$PENV_HOME` refuses naming `penv install`, rather than reporting the package as one penv has never heard of.
...
|---|---|
| Official (`@penvhq/*`) | Nothing. Provenance is verified, the manifest records version and integrity, and the only question is the one config edit — asked once, naming every environment it would repoint. |
...
`add` takes four flags and no others: `--registry <https-url>` names a private registry, which is what puts a package in the private tier, `--trust-young` is the override for the seven-day age gate, `--local` is the development path below, and `--yes` says nobody is here to be asked. Pin a version with `penv add <package>@<version>`; without one, `add` takes what `latest` points at and records the exact version it resolved.

**An add with no questions runs unattended.** Because the official tier asks nothing, `penv add @penvhq/provider-vercel` finishes with no terminal at all — the two offers print their advice instead of asking, and `--yes` is how you get that same run at a real terminal. What no flag can supply is the trust ceremony's one field, a sentence about why a stranger's code is trusted, so anything outside `@penvhq/*` refuses unattended and names it. `--local` still refuses in CI: what it records is what this checkout builds, which is not a pipeline's decision.

**A repository that writes a provider uses `penv add --local`.** There is no release to pin — the package is the one this checkout builds — so nothing goes in the manifest, whose whole promise is that every machine gets the reviewed bytes. What `--local` does is resolve the package from the project's own `node_modules`, import it once for the same load check, write the same type-only declaration, and record the name in `.penv/state/local-extensions.json`: a committed list of names, nothing else. That record is the point. `penv doctor` reports every local extension with the `?` verdict, because penv cannot say the adapter here is the adapter anyone else gets, and CI refuses one outright, naming `penv add <package>` — a pipeline runs bytes that were pinned or it runs none. `--local` takes no version, no `--registry`, and no `--trust-young`: all three describe a release, and there isn't one.
...
| `penv add <package>[@<version>]` | Add a provider extension: record it in the manifest with its integrity, install it into the launcher's cache, generate its type declaration, offer the config edit and any onboarding step. `--registry <url>` for a private registry; `--trust-young` overrides the seven-day age gate; `--local` records a provider this repository builds, pinning nothing; `--yes` asks nothing, which is what an official add does unattended anyway. |
| `penv upgrade [version]` | Move the pinned engine and every `@penvhq/penv` dependency in the project to the same exact version. No version takes whatever `latest` points at. Shows every file change and asks once; `--yes` answers in advance, and an unattended run needs it *and* an explicit version. Extensions keep their own pins. |
